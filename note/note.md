...
console.log(multiply((multiply((multiply(5))))));
// expected output: 5
...
- 数组
- Set 和 Map 结构
- 某些类似数组的对象（比如`arguments`对象、DOM NodeList 对象）、Generator 对象，以及字符串
...
**arr[Symbol.iterator].bind(arr)  返回arr的这个属性**
...
调用 Generator 函数后，该函数并不执行，返回的也不是函数运行结果，而是一个指向内部状态的指针对象，也就是刚刚说的遍历器对象（Iterator Object）。
